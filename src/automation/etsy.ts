...
import { Credentials, ActivityLogFilter, EtsyPerson } from '../types';
...
export const loginEtsyUser = (credentials: Credentials) => async (
  ...
  if (signInSignal) {
    ...
    await Promise.all([
      ...
      await page.waitForFunction(`document.querySelector('[data-appears-component-name*="WelcomeRow"]').innerText.includes("Welcome back")`)
  }
};
 ...
 */
export const navigateToEtsyPeoplePage = (etsyPerson:EtsyPerson) => async (page: Page): Promise<void> => {
    const SELECTORS = {
    ...
    const convoTriggerElement = await page.waitForSelector(SELECTORS.CONVO_TRIGGER_SELECTOR)
    ...
    await page.waitFor(WAIT_RANDOM),
    await page.keyboard.type(MARKETING_MESSAGE)
};
